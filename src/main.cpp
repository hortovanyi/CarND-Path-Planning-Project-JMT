...
//  }
  ...
  cout << "way points " << highway_map.way_points().size()<< endl;
  for (auto wp: highway_map.way_points()){
//    cout << wp.x << " " << wp.y << " " << wp.s << " next " << wp.next->s << " dist " << wp.waypoint_distance(*wp.next) << endl;
    cout << wp->x << " " << wp->y << " " << wp->s << " wp * " << wp<< " wp.next " << wp->next->x << " " << wp->next->y << " ";
    cout << "dist " << wp->waypoint_distance(*wp->next) << endl;
  }
...
//                     uWS::OpCode opCode) {
  h.onMessage(
      [&highway_map](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
          uWS::OpCode opCode) {
